...
void anti_adblock_init(void) {
    /* SECURITY FIX: Use cryptographically strong random seed
     * Fallback chain: arc4random()/getrandom() → /dev/urandom → time+pid */
    unsigned int seed = 0;

#ifdef __FreeBSD__
    /* FreeBSD: Use arc4random_buf (cryptographically secure, never blocks)
     * Available since FreeBSD 3.0 (1998), uses ChaCha20 since FreeBSD 12.0 */
    ...
    /* Final fallback: Combine multiple sources */
    struct timeval tv;
    gettimeofday(&tv, NULL);
    seed = (unsigned int)(tv.tv_sec ^ tv.tv_usec ^ getpid() ^ (uintptr_t)&seed);
    srand(seed);
}
...
adblock_seed_t anti_adblock_generate_seeds(const char *request_uri,
    ...
    adblock_seed_t seeds = {0};  /* Initialize to zero - fixes Valgrind warning */
    struct timeval tv;
...
void anti_adblock_generate_cors(const char *origin, cors_config_t *cors) {
    ...
    if (origin && *origin != '\0') {
        ...
        strncpy(cors->origin, "*", sizeof(cors->origin) - 1);
        cors->origin[sizeof(cors->origin) - 1] = '\0';
        cors->has_origin = 1;
        cors->allow_credentials = 0;
    }
...
/* Randomized CORS - Anti-Fingerprinting Decision */
int anti_adblock_should_send_cors(const adblock_seed_t *seeds) {
    if (!seeds) return 0;

    /* Use crypto_seed for probabilistic decision
...
/* Generate randomized CORS header */
void anti_adblock_generate_random_cors(const adblock_seed_t *seeds,
    ...
    if (!origin_header || *origin_header == '\0') {
        ...
        /* Use request_seed bit pattern for decision
         * 50% wildcard, 50% specific origin
         */
    ...
    if (use_wildcard) {
        ...
        /* Specific origin - sometimes with credentials */
...
const char* anti_adblock_get_csp_policy(csp_policy_type_t policy_type,
    ...
    const int num_policies = (int)(sizeof(CSP_POLICIES) / sizeof(CSP_POLICIES[0]));
    if (policy_type >= 0 && policy_type < num_policies) {
