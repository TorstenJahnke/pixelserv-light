 ...
 * for correct wildcard certificate generation in DNS sinkhole context.
 *
 * Purpose: Generate wildcards at the PUBLIC SUFFIX level to catch ALL subdomains
 *
 * Example (github.io is a 2nd-level TLD):
 *   mysite.github.io → *.github.io (catches ALL github.io sites!)
 *
 * Example (co.uk is a 2nd-level TLD):
 *   www.example.co.uk → *.example.co.uk (not *.co.uk - that's the TLD itself)
 *
 * Features:
 * - Load from external file (e.g., /opt/aviontex/config/second-level-tlds.conf)
 * - O(1) hash-based lookup for fast domain classification
 * - Handles 10,000+ entries efficiently (Public Suffix List)
 */
