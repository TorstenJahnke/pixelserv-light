/*
 * cert_index.h - Lock-Free Certificate Index System for 4-8M Certificates
 *
 ...
 * - 100% LOCK-FREE: reads AND writes (20,000+ ops/sec)
 * - Per-shard atomic counters (no global lock)
 * - Per-shard log files (256 files, no contention)
 * - O_APPEND atomic writes (18 bytes < sector size)
 *
 * Directory Layout (e.g. /opt/aviontex):
 *   RSA/
 *     certs/00..ff/cert_*.pem
 *     index/.index (binary-indexed, mmap'd)
 *     rootCA/subca.crt, subca.key, rootca.crt
 *   ECDSA/
 *     certs/00..ff/
 *     index/.index
 *     rootCA/subca.crt, subca.key, rootca.crt
 ...
 *     index/.index
 *     rootCA/subca.crt, subca.key, rootca.crt
 *   LEGACY/  (RSA1024-SHA1, Root-only signing)
 *     certs/00..ff/
 *     index/.index
 *     rootCA/rootca.crt, rootca.key (no SubCA)
 *
 ...
 *
 * Returns a unique cert_id atomically using per-shard counters.
 * Safe to call from multiple threads simultaneously.
 *
 * @param index     Index handle
 * @param shard_id  Shard ID (0-255)
 * @return Unique cert_id, 0 on error
 */
uint32_t cert_index_alloc_cert_id(cert_index_t *index, uint8_t shard_id);

/**
 * Register new certificate in index (LOCK-FREE)
 *
 * Uses O_APPEND atomic writes to per-shard log files.
 * Safe to call from multiple threads (20,000+ ops/sec).
 *
 * @param index     Index handle
...
/**
 * Mark certificate as expired/revoked (LOCK-FREE)
 *
 * Appends deletion marker to per-shard log.
 *
 * @param index     Index handle
 * @param domain    Domain name
