...
/* Forward declaration */
static void cache_clear_hash_slot(int entry_idx, const char *cert_name);
...
void cache_init(int size) {
    ...
    for (int i = 0; i < cache_size; i++) {
        ...
        atomic_store(&cache_entries[i].expired_at, 0);
...
int cache_insert(const char *cert_name, SSL_CTX *sslctx) {
    ...
    /* Find a free entry slot (prefer EMPTY, fall back to old EXPIRED) */
    int entry_idx = -1;
    int expired_idx = -1;
    uint32_t now = (uint32_t)time(NULL);
    uint32_t grace_threshold = now - CACHE_RECLAIM_GRACE_SECONDS;

    for (int i = 0; i < cache_size; i++) {
        int state = atomic_load(&cache_entries[i].state);
        ...
        } else if (state == CACHE_EXPIRED && expired_idx < 0) {
            /* Remember first expired entry in case we need it */
            uint32_t expired_at = atomic_load(&cache_entries[i].expired_at);
            if (expired_at > 0 && expired_at < grace_threshold) {
                expired_idx = i;
            }
        }
    }

    /* If no EMPTY found, try to reclaim an old EXPIRED entry */
    if (entry_idx < 0 && expired_idx >= 0) {
        cache_entry_t *entry = &cache_entries[expired_idx];
        int expected = CACHE_EXPIRED;
        if (atomic_compare_exchange_strong(&entry->state,
                                           &expected, CACHE_INSERTING)) {
            /* We own this entry now, free old resources */
            char *old_name = entry->cert_name;
            SSL_CTX *old_ctx = entry->sslctx;

            /* Clear hash table slot for old entry */
            if (old_name) {
                cache_clear_hash_slot(expired_idx, old_name);
                free(old_name);
            }
            if (old_ctx) {
            ...
            entry->sslctx = NULL;
            entry->alloc_len = 0;
            atomic_fetch_sub(&cache_used, 1);

            entry_idx = expired_idx;
            log_msg(LGG_DEBUG, "Recycled expired slot %d for %s", entry_idx, cert_name);
        }
    }
    ...
    memcpy(entry->cert_name, cert_name, entry->alloc_len);
    entry->sslctx = sslctx;
    atomic_store(&entry->last_use, (uint32_t)time(NULL));
    ...
    for (int i = 0; i < cache_size; i++) {
        ...
        if (state == CACHE_VALID) {
            ...
            if (last_use < threshold) {
                /* Try to expire - just mark, don't free yet (grace period) */
                int expected = CACHE_VALID;
                if (atomic_compare_exchange_strong(&entry->state,
                                                   &expected, CACHE_EXPIRED)) {
                    /* Record when we expired this entry for grace period tracking */
                    atomic_store(&entry->expired_at, now);
                    stats_inc_purge();
...
/* Helper: Clear hash table slot pointing to entry_idx */
static void cache_clear_hash_slot(int entry_idx, const char *cert_name) {
    if (!cert_name || !hash_table) return;
    ...
    for (int i = 0; i < hash_table_size; i++) {
        ...
        if (idx == entry_idx) {
            /* Found it - clear with CAS (might fail if concurrent insert, that's OK) */
            int expected = entry_idx;
            atomic_compare_exchange_strong(&hash_table[slot], &expected, -1);
            return;
        }
    }
}
...
void cache_reclaim_expired(void) {
    ...
    for (int i = 0; i < cache_size; i++) {
        ...
        if (state == CACHE_EXPIRED) {
            uint32_t expired_at = atomic_load(&entry->expired_at);

            /* Only reclaim if grace period has passed */
            if (expired_at > 0 && expired_at < grace_threshold) {
                /* Try to transition EXPIRED -> EMPTY (single reclaimer) */
                int expected = CACHE_EXPIRED;
                if (atomic_compare_exchange_strong(&entry->state,
                                                   &expected, CACHE_INSERTING)) {
                    /* We own this entry now, safe to free */
                    char *old_name = entry->cert_name;
                    SSL_CTX *old_ctx = entry->sslctx;

                    /* Clear hash table slot BEFORE clearing entry */
                    if (old_name) {
                        cache_clear_hash_slot(i, old_name);
                    }
                    ...
                    entry->sslctx = NULL;
                    entry->alloc_len = 0;
                    atomic_store(&entry->expired_at, 0);
                    ...
                    log_msg(LGG_DEBUG, "Reclaimed cache slot %d", i);
                }
            }
        }
    }
